...
class UserService extends ChangeNotifier {
  ...
  Future<void> init() async {
    ...
    try {
      ...
      debugPrint(
          "[UserService] init: Loaded from SharedPreferences -> userId: $userId, username: $username, isGlobalMute: $isGlobalMute");
      debugPrint("[UserService] init: UserService initialization complete.");
    } catch (e, stackTrace) {
      debugPrint("[UserService] init: ERROR during initialization: $e");
      debugPrintStack(
          stackTrace: stackTrace, label: "[UserService] init Error");
  ...
  bool hasUser() {
    ...
    debugPrint(
        "[UserService] hasUser: Checking if user exists. UserID: $userId, Username: $username. Result: $result");
    return result;
  }

  Future<bool> createUser(String newUsername) async {
    debugPrint(
        "[UserService] createUser: Attempting to create user with username: '$newUsername'");
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint(
            "[UserService] createUser: User ID received: $userId. Setting username to: $newUsername");

        if (userId != null) {
          await _prefs.setString('userId', userId!);
          await _prefs.setString('username', username!);
          debugPrint(
              "[UserService] createUser: User details saved to SharedPreferences. UserId: $userId, Username: $username");
          notifyListeners();
          debugPrint(
              "[UserService] createUser: Listeners notified. User creation successful.");
          return true;
        } else {
          debugPrint(
              "[UserService] createUser: Failed to create user - received null userId from server.");
          return false;
        }
      } else {
        debugPrint(
            "[UserService] createUser: Failed to create user - server responded with status ${response.statusCode}");
      }
    } catch (e, stackTrace) {
      debugPrint('[UserService] createUser: ERROR creating user: $e');
      debugPrintStack(
          stackTrace: stackTrace, label: '[UserService] createUser Error');
    }
  ...
  Future<void> updateUsername(String newUsername) async {
    ...
    try {
      ...
      debugPrint(
          "[UserService] updateUsername: ERROR saving username to SharedPreferences: $e");
      debugPrintStack(
          stackTrace: stackTrace, label: '[UserService] updateUsername Error');
    }
  }

  Future<void> updateUserId(String newUserId) async {
    debugPrint(
        "[UserService] updateUserId: Attempting to update userId to: '$newUserId'. Current userId: $userId");
    userId = newUserId;
    try {
      await _prefs.setString('userId', userId!);
      debugPrint(
          "[UserService] updateUserId: UserId '$newUserId' saved to SharedPreferences.");
      notifyListeners();
      debugPrint("[UserService] updateUserId: Listeners notified.");
    } catch (e, stackTrace) {
      debugPrint(
          "[UserService] updateUserId: ERROR saving userId to SharedPreferences: $e");
      debugPrintStack(
          stackTrace: stackTrace, label: '[UserService] updateUserId Error');
    }
  }
  ...
  Future<void> toggleGlobalMute() async {
    ...
    try {
      ...
      debugPrint(
          "[UserService] toggleGlobalMute: ERROR saving global mute state: $e");
      debugPrintStack(
          stackTrace: stackTrace,
          label: '[UserService] toggleGlobalMute Error');
    }
  }

  Future<bool> syncContactsPseudos(List<Contact> localContacts) async {
    debugPrint(
        "[UserService] syncContactsPseudos: Attempting to sync pseudos for ${localContacts.length} contacts.");
    if (localContacts.isEmpty) {
      debugPrint(
          "[UserService] syncContactsPseudos: No local contacts to sync. Returning false.");
      return false;
    }
    ...
    try {
      ...
      debugPrint(
          "[UserService] syncContactsPseudos: UserIds to sync: $userIds");
      ...
      if (response.statusCode == 200) {
        ...
        for (var contact in localContacts) {
          if (remotePseudos.containsKey(contact.userId) &&
            ...
            debugPrint(
                "[UserService] syncContactsPseudos: Contact ${contact.userId} not found in remote pseudos response.");
          } else {
        ...
        if (hasBeenUpdated) {
          debugPrint(
              "[UserService] syncContactsPseudos: Contacts updated. Notifying listeners.");
          notifyListeners(); // Notify if any contact was actually updated
        } else {
          debugPrint(
              "[UserService] syncContactsPseudos: No contact pseudos needed updating.");
        }
      } else {
        debugPrint(
            "[UserService] syncContactsPseudos: Failed to get pseudos - server responded with status ${response.statusCode}");
      }
    } catch (e, stackTrace) {
      debugPrint('[UserService] syncContactsPseudos: ERROR during sync: $e');
      debugPrintStack(
          stackTrace: stackTrace,
          label: '[UserService] syncContactsPseudos Error');
    }
    debugPrint(
